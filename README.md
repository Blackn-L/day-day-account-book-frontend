# 呆呆记账本
> 自己写的移动端记账本，Vue3 + Typescript + Vite + Vant4，可以作为该类技术栈项目搭建的参考
...
* `less`
* `unplugin-vue-components` + `@vant/auto-import-resolver` 按需引入插件，实现 Vant 的自动按需引入
...
### 插件变更
- 移除 `vite-plugin-style-import`（已废弃）
- 使用 `unplugin-vue-components` + `@vant/auto-import-resolver` 实现自动按需引入
- 移除 `lib-flexible`（不再维护）

### 配置优化
- 组件自动导入，无需手动注册 Vant 组件
- 代理地址改为本地开发地址
